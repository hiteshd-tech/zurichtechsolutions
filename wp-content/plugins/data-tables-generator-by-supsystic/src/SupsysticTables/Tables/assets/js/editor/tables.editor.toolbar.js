...
(function ($, app, undefined) {
        ...
        $('#insertUrlDialog').dialog({
          ...
          buttons: {
            Insert: function (e) {
              ...
              editor.setDataAtCell(selection.highlight.row, selection.highlight.col, insertToField);
              $this.dialog('close');
            },
            Cancel: function (e) {
              $(this).dialog('close');
  ...
  var Toolbar = (function () {
      ...
      switch (type) {
        ...
        case 'video':
          if (isEmbed) {
            ...
            content += '<source src="' + url + '" ' + (typeof attachment.mime != 'undefined' ? 'type="' + attachment.mime + '"' : '') + '>';
            content += '</video></div>';
          } else if (linkHtml) {
            content = linkHtml;
          }
    ...
    Toolbar.prototype.subscribe = function () {
      ...
      // Set methods
      this.getContainer()
        .find('button, .toolbar-content > a, .tool')
        .each(function () {
          var $button = $(this);
          ...
          if ($button.data('method') !== undefined && methods[$button.data('method')] !== undefined) {
            ...
            $button.on(event, function (e) {
              ...
              if (/word-wrap-default|word-wrap-visible|word-wrap-hidden/.test(method)) {
                $('#toolbar-word-wrapping i').removeClass('word-wrap-visible word-wrap-hidden').addClass(method);
              }
              if (method == 'setFormat') {
