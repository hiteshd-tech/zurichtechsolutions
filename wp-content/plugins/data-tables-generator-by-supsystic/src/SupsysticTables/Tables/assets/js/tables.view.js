  ...
  $(document).ready(function () {
            ...
            for (var i = 0; i < fields.length; i++) {
              if (typeof fields[i].value != 'undefined') {
                var fieldRow = fieldRowExample.clone(),
                  name = fields[i].label ? fields[i].label + ' (' + fields[i].name + ')' : fields[i].placeholder ? fields[i].placeholder + ' (' + fields[i].name + ')' : fields[i].name;
    ...
    $('.item-inner').on('click', function () {
      ...
      if ($tblLoaderIconName.val() == 'default') {
        ...
        $tblLoaderPreview.append('<div class="supsystic-table-loader la-' + $tblLoaderIconName.val() + ' la-2x" style="color:' + $tblLoaderIconColor + '">' + items + '</div>');
      }
