(function ($, app, undefined) {
  var Formula = (function () {
    ...
    Formula.prototype.subscribe = function () {
      this.getEditor().addHook(
        'afterSelection',
        $.proxy(function () {
          var range = this.getEditor().getSelectedRangeLast(),
            row = editor.toPhysicalRow(range.highlight.row),
            sourceDataAtCell = this.getEditor().getSourceDataAtCell(row, range.highlight.col);

          this.setValue(sourceDataAtCell);
        }, this)
      );
          ...
          if (!self.getInputTimeout()) {
            setTimeout(function () {
              ...
              if (keyupData.oldVal != keyupData.newVal) {
                self.getEditor().setDataAtCell(keyupData.row, keyupData.col, keyupData.newVal);
              }
