...
(function ($, app) {
  var TablesModel = (function () {
    ...
    TablesModel.prototype.getPartRows = function (id, limit, offset, deferred) {
      var self = this;
      $.when(this.request('getRows', { id: id, limit: limit, offset: offset })).done(function (rowsResponse) {
          ...
          $.each(row.cells, function (y, cell) {
            ...
            if ('meta' in cell && cell.meta !== undefined) {
              ...
              if (null !== fontFamily && $style.html().indexOf(fontFamily[0]) == -1) {
                ...
                if (g_stbStandartFontsList && toeInArray(family, g_stbStandartFontsList) == -1 && g_stbAllFontsList && toeInArray(family, g_stbAllFontsList) != -1) {
    ...
    TablesModel.prototype._saveTable = function (preview, rerender) {
      ...
      if (preview !== false) {
        ...
        if ($table.length) {
          ...
          $(preview)
            .html($('<i/>', { class: 'fa fa-spinner fa-spin' }).attr('style', 'font-size: 2em !important'))
            .prepend('<label> Table generate in process.... </label>');
        }
      } else {
        g_stbNeedPreview = true;
      }
      ...
      if (g_stbDoSaving) {
        ...
        if (!g_stbDoPreview) {
          ...
          if (container.length) {
            ...
            return this.render(app.getParameterByName('id'), settings)
      ...
      for (var i = 0; i < selectors.length; i++) {
        ...
        if (newRules.length) {
          ...
          if (param == 'font-family') {
            if (g_stbStandartFontsList && toeInArray(value, g_stbStandartFontsList) == -1 && g_stbAllFontsList && toeInArray(value, g_stbAllFontsList) != -1) {
    ...
    TablesModel.prototype.render = function (id, settings) {
      ...
      return this.request('render', { id: id, settings: typeof settings == 'undefined' ? false : settings, preview: '1' }).done(function () {
        setTimeout(function () {
          $('#settings').css({ pointerEvents: 'auto' });
        }, 700);
      });
    };
      ...
      if (value && app.isNumber(value) && !isNaN(value)) {
        ...
        switch (formatType) {
          ...
          case 'currency':
            ...
            // We need to use dafault delimiters for format string
            preparedFormat = format.replace(formatWithoutCurrency, formatWithoutCurrency.replace(delimiters[0], '.').replace(delimiters[1], ',')).replace(currencySymbol, '$');
    ...
    TablesModel.prototype._afterTablePreview = function (table) {
      if (SDT_DATA.isPro && typeof app.createEditableFields != 'function') {
        // for compatibility with old pro versions
