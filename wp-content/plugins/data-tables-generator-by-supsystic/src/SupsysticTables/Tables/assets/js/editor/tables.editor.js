...
(function ($, app, undefined) {
  $(document).ready(function () {
    ...
    Handsontable.editors.TextEditor.prototype.saveValue = function (val, ctrlDown) {
      ...
      var type = this.cellProperties.type || '',
        formatType = this.cellProperties.formatType || '',
        value = val[0][0];
    ...
    Handsontable.editors.DropdownEditor.prototype.beginEditing = function () {
      // To show percents as is if it is pure number
      var formatType = this.cellProperties.formatType || '',
        source = this.cellProperties.source || [];
    ...
    $.when(tablesModel.getMeta(tableId), tablesModel.getCountRows(tableId))
      .done(function (metaResponse, countResponse) {
        $.when(tablesModel.getRows(tableId, countResponse[0].countRows))
    ...
    editor.setPageData = (function () {
      return function (inBuffer) {
        ...
        var stop = page * limit - 1,
          heights = [];
        ((partData = []), (mergeCells = []), (rowHeaders = []), (links = $('#pagination a')));
