...
$features = isset($table->settings['features']) ? json_encode_escape(array_keys($table->settings['features'])) : json_encode([]);
...
<div class="supsystic-tables-wrap">
	<table
		...
		<?php endif; ?>
		<?php
/* Datatables requred thead section declared */
?>
		<thead>
			<tr>
				<?php foreach ($table->rows[0]['cells'] as $cellIndex => $cell) {
      if ($header) {
