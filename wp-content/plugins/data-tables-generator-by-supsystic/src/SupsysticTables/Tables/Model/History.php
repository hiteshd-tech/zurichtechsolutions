...
class SupsysticTables_Tables_Model_History extends SupsysticTables_Core_BaseModel
{
  ...
  {
    $query = $this->getQueryBuilder()->select('*')->from($this->getTable('rows_history'))->where('table_id', '=', (int) $tableId);
  ...
  {
    $query = $this->getQueryBuilder()->select('*')->from($this->getTable('rows_history'))->where('table_id', '=', (int) $tableId)->andWhere('user_id', '=', (int) $userId);
    $history = $this->db->get_results($query->build());
  ...
  public function updateUserTableHistory($userId, $tableId, $data, $period = null)
  {
    ...
    if ($historyTable) {
      $query = $this->getQueryBuilder()->update($this->getTable('rows_history'))->fields(array_keys($history))->values(array_values($history))->where('id', '=', (int) $historyTable->id);
  ...
  {
    $query = $this->getQueryBuilder()->select('data')->from($this->getTable('rows'))->where('table_id', '=', (int) $tableId)->orderBy('id')->order('ASC');
    ...
    $query = $this->getQueryBuilder()->insertInto($this->getTable('rows_history'))->fields(array_keys($history))->values(array_values($history));
    $this->db->get_results($query->build());
    ...
    $query = $this->getQueryBuilder()->select('*')->from($this->getTable('rows_history'))->where('id', '=', (int) $this->db->insert_id);
  ...
  {
    $query = $this->getQueryBuilder()->select('history_settings')->from($this->getTable('tables'))->where('id', '=', (int) $id);
  ...
  public function setHistorySettings($id, $settings)
  {
    $query = $this->getQueryBuilder()
      ...
      ->where('id', '=', (int) $id)
      ->set(['history_settings' => serialize($settings)]);
