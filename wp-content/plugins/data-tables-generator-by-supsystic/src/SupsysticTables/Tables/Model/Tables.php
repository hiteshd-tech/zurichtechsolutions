...
class SupsysticTables_Tables_Model_Tables extends SupsysticTables_Core_BaseModel
{
  ...
  {
    $query = $this->getColumnQuery($id)->where($this->getField('columns', 'index'), '=', (int) $index);
  ...
  {
    $query = $this->getColumnQuery($id)->orderBy($this->getField('columns', 'index'));
  ...
  public function addColumn($id, $column)
  {
    if (!is_array($column) && !is_object($column)) {
      throw new InvalidArgumentException(sprintf('Second parameter must be an array or an instance of the stdClass, %s given.', gettype($column)));
    }
    ...
    $query = $this->getQueryBuilder()->insertInto($this->getTable('columns'))->fields(array_keys($column))->values(array_values($column));
  ...
  public function setColumn($id, $index, $column)
  {
    ...
    $query = $this->getQueryBuilder()->update($this->getTable('columns'))->fields(array_keys($column))->values(array_values($column))->where($this->getField('columns', 'table_id'), '=', (int) $id)->andWhere($this->getField('columns', 'index'), '=', (int) $index);
  ...
  {
    $query = $this->getQueryBuilder()->deleteFrom($this->getTable('columns'))->where('table_id', '=', (int) $id);
  ...
  public function addRow($id, array $data)
  {
    ...
    $query = $this->getQueryBuilder()->insertInto($this->getTable('rows'))->fields($this->getField('rows', 'table_id'), $this->getField('rows', 'data'))->values((int) $id, serialize($data));
  ...
  public function stripos_arr($haystack, $needle)
  {
    ...
    foreach ($needle as $what) {
      if (is_string($haystack) && ($pos = stripos($haystack, $what)) !== false) {
        return $pos;
      }
    }
    return false;
  ...
  {
    $query = $this->getQueryBuilder()->select($this->getField('rows', 'data'))->from($this->getTable('rows'))->where('table_id', '=', (int) $id)->orderBy($this->getField('rows', 'id'));
  ...
  {
    $query = $this->getQueryBuilder()->select('id, data')->from($this->getTable('rows'))->where('table_id', '=', (int) $id);
  ...
  public function getNeededRows($id, &$settings, $isSSP, $attributes = false, $export = false)
  {
    ...
    if ($isSSP) {
      $cntHead = 1;
      $footers = [];
      if (isset($settings['elements']['head']) && $settings['elements']['head'] == 'on' && isset($settings['headerRowsCount']) && $settings['headerRowsCount'] > 0) {
        $headers = $this->getRows($id, $settings['headerRowsCount']);
      } else {
        $headers = $this->getRows($id, $cntHead);
      }
      if (isset($settings['elements']['foot']) && $settings['elements']['foot'] == 'on' && isset($settings['customFooter']) && $settings['customFooter'] == 'on' && isset($settings['footerRowsCount']) && $settings['footerRowsCount'] > 0) {
  ...
  public function getRowsByIds($id, &$settings, $ids, $attributes = false)
  {
    ...
    $query = $this->getQueryBuilder()->select($this->getField('rows', 'data'))->from($this->getTable('rows'))->where('table_id', '=', (int) $id)->orderBy($this->getField('rows', 'id'))->limit($limit);
    do {
      $query->offset($offset);
  ...
  {
    $query = $this->getQueryBuilder()->select('count(*)')->from($this->getTable('rows'))->where('table_id', '=', (int) $id);
  ...
  public function getRowsByPart($id, $searchAll, $searchCols, $orderCol, $orderAsc, $start, $length, $header, $footer, $searchParams, $table)
  {
    ...
    $query = $this->getQueryBuilder()->select('id, data')->from($this->getTable('rows'))->where('table_id', '=', (int) $id)->orderBy($this->getField('rows', 'id'));
  ...
  {
    $query = $this->getQueryBuilder()->deleteFrom($this->getTable('rows'))->where($this->getField('rows', 'table_id'), '=', (int) $id)->andWhere($this->getField('rows', 'id'), '<=', (int) $lastRowId);
  ...
  public function removeLastRows($id, $count)
  {
    ...
    $query = $this->getQueryBuilder()->deleteFrom($this->getTable('rows'))->where($this->getField('rows', 'table_id'), '=', (int) $id)->andWhere($this->getField('rows', 'id'), '>', (int) ($lastRowId[0] - $count));
  ...
  {
    $query = $this->getQueryBuilder()->deleteFrom($this->getTable('rows'))->where($this->getField('rows', 'table_id'), '=', (int) $id);
  ...
  public function setMeta($id, array $meta)
  {
    $query = $this->getQueryBuilder()
      ...
      ->where('id', '=', (int) $id)
      ->set(['meta' => htmlspecialchars(serialize($meta), ENT_QUOTES)]);
  ...
  public function onTablesGetAll($tables)
  {
    ...
    return array_map([$this, 'onTablesGet'], $tables);
  }
  ...
  public function onInstanceReady()
  {
    ...
    $dispatcher->on('tables_get', [$this, 'onTablesGet']);
    $dispatcher->on('tables_get', [$this, 'onTablesGetPro']);
    $dispatcher->on('tables_remove', [$this, 'onTablesRemove']);
  ...
  {
    return $this->getQueryBuilder()->select($this->getField('columns', 'title'))->from($this->getTable('columns'))->where('table_id', '=', (int) $id);
  }
  ...
  {
    $query = $this->getQueryBuilder()->select($this->getField('tables', 'settings'))->from($this->getTable('tables'))->where('id', '=', (int) $id);
  ...
  {
    $query = $this->getQueryBuilder()->select($this->getField('tables', 'meta'))->from($this->getTable('tables'))->where('id', '=', (int) $id);
