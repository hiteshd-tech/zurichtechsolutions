...
class SupsysticTables_Tables_Module extends SupsysticTables_Core_BaseModule
{
   ...
   */
  protected $isSingleCell = [];
  protected $isTablePart = [];
  /**
   * Data for loading tables' rows from history
   */
  public $isFromHistory = [];
  public $historyData = [];
  /**
   ...
   */
  private $tableSearch = '';
  private $shortAttributes = [];
  /**
   * Variables for appending of table styles to site header
   */
  private $_tablesInPosts = [];
  private $_tablesObj = [];
  private $_tablesStyles = [];
  ...
  public function onInit()
  {
    ...
    $dispatcher->on('before_table_render', [$this, 'loadLanguageData']);
    ...
    add_action('wp_head', [$this, 'setDataTableStyles']);
    add_action('widgets_init', [$this, 'registerWidget']);
    add_action('shutdown', [$this, 'onShutdown']);
    $dispatcher = $this->getEnvironment()->getDispatcher();
    $dispatcher->on('after_tables_loaded', [$this, 'onAfterLoaded']);
    $this->renderTableProSections();

    add_filter('jetpack_lazy_images_blacklisted_classes', [$this, 'excludeFromLazyLoad'], 999, 1);
  }
  ...
  public function registerSearchFilter()
  {
    if (!is_admin()) {
      ...
      if (!empty($settings['table_search'])) {
        add_filter('posts_search', [$this, 'globalTablesSearchFilter'], 10, 2);
      }
    }
  }
  ...
  public function render($id, $settings = false)
  {
    ...
    if ($this->isSingleCell) {
      ...
      $table->meta['css'] =
        ...
        ' #supsystic-table-' .
        $id .
        ' td { width: auto !important; min-width: 100px; }';
      ...
      foreach ($table->rows as $key => $row) {
        if (empty($this->isTablePart['row']) || in_array($key + 1, $this->isTablePart['row'])) {
          foreach ($row['cells'] as $index => $cell) {
            ...
            } elseif (!empty($this->isTablePart['col'])) {
              unset($table->rows[$key]['cells'][$index]);
            }
          }
        } elseif (!empty($this->isTablePart['row'])) {
          unset($table->rows[$key]);
        }
      }
    }
    ...
    $dispatcher->apply('before_table_render', [$table]);
    if ($table->isSSP) {
      $dispatcher->apply('before_table_render_ssp', [$table]);
    }
    $searchValue = '';
          ...
          if (!empty($table->meta['mergedCells'])) {
            foreach ($table->meta['mergedCells'] as $m) {
              if ($m['row'] == $key && $m['col'] == $index && $m['rowspan'] > 1) {
                ...
                $footer[$newKey]['cells'][$index]['rewrite'] = [
                  'rowspan' => $m['rowspan'],
                  'colspan' => $m['colspan'],
                  'display' => true,
                ];
                $footer[$key]['cells'][$index]['rewrite'] = [
                  'rowspan' => 1,
                  'colspan' => 1,
                  'display' => false,
                ];
  ...
  private function _getTypeFormats($table)
  {
    ...
    if (!empty($table->settings['useNumberFormat']) && $table->settings['useNumberFormat']) {
      ...
      // date format
      if ($dateFormat) {
        $formats['date']['format'] = strtr($dateFormat, [
          'YYYY-MM-DD' => 'Y-m-d',
          'DD.MM.YYYY' => 'd.m.Y',
          'HH:mm' => 'H:i',
          'hh:mm a' => 'h:i a',
      ...
      if ($timeDurationFormat) {
        $formats['time']['format'] = strtr($timeDurationFormat, [
          'HH:mm' => 'H:i',
          'hh:mm a' => 'h:i a',
        ]);
      }
  ...
  public function doShortcode($attributes)
  {
    ...
    $assets = array_filter($ui->getAssets(), [$this, 'filterAssets']);

    if (count($assets) > 0) {
      foreach ($assets as $asset) {
        add_action('wp_footer', [$asset, 'load']);
      }
    }
  ...
  public function doValueShortcode($attributes)
  {
    ...
    if (!array_key_exists('id', $attributes) || !array_key_exists('row', $attributes) || !array_key_exists('col', $attributes)) {
      return $environment->translate('There are not all shortcode\'s attributes specified. Usage example') . ':<br />' . sprintf('[%s id="{table id}" row="{row number}" col="{column number}"]', $shortcode);
    }
  ...
  public function doCellShortcode($attributes)
  {
    ...
    if (!array_key_exists('id', $attributes) || !array_key_exists('row', $attributes) || !array_key_exists('col', $attributes)) {
      return $environment->translate('There are not all shortcode attributes specified. Usage example') . ':<br />' . sprintf('[%s id="{table id}" row="{row number}" col="{column number}"]', $shortcode);
    }
    $ui = $environment->getModule('ui');
    $assets = array_filter($ui->getAssets(), [$this, 'filterAssets']);
    ...
    $this->isSingleCell = ['row' => (int) $attributes['row'], 'col' => (int) $this->_lettersToNumbers($attributes['col'])];
  ...
  public function doTablePartShortcode($attributes)
  {
    ...
    if (!array_key_exists('id', $attributes) || (!array_key_exists('row', $attributes) && !array_key_exists('col', $attributes))) {
      return $environment->translate('There are not all shortcode attributes specified. Usage example') . ':<br />' . sprintf('[%s id="{table id}" row="{row numbers splitted by comma}" col="{column numbers splitted by comma}"]', $shortcode);
    }
    $ui = $environment->getModule('ui');
    $assets = array_filter($ui->getAssets(), [$this, 'filterAssets']);
    ...
    $rowsResult = [];
    $rows = array_filter(explode(',', $attributes['row']));
    foreach ($rows as $row) {
      if (strpos($row, '-') !== false) {
    ...
    $colsResult = [];
    $cols = array_filter(explode(',', $attributes['col']));
    foreach ($cols as $col) {
      if (strpos($col, '-') !== false) {
  ...
  protected function registerShortcodes()
  {
    ...
    add_shortcode($config->get('shortcode_name'), [$this, 'doShortcode']);
    add_shortcode($config->get('shortcode_part_name'), [$this, 'doTablePartShortcode']);
    add_shortcode($config->get('shortcode_cell_name'), [$this, 'doCellShortcode']);
    add_shortcode($config->get('shortcode_value_name'), [$this, 'doValueShortcode']);
  }
  ...
  private function registerTwigTableRender()
  {
    ...
    $twig->addFunction(new Twig_SupTwgDtgs_SimpleFunction('render_table', $callable, ['is_safe' => ['html']]));
  }
  ...
  private function addTwigHighlighter()
  {
    ...
    $twig->addFilter(new Twig_SupTwgDtgs_SimpleFilter('highlight', 'highlight_string', ['is_safe' => ['html']]));
  }
  ...
  public function afterUiLoaded(SupsysticTables_Ui_Module $ui)
  {
    ...
    $ui->add($ui->createScript('supsystic-tables-datatables-numeral')->setHookName($dynamicHookName)->setModuleSource($this, 'libraries/numeral.min.js')->setVersion($version)->setCachingAllowed(true)->addDependency('jquery')->addDependency('supsystic-tables-datatables-js'));

    $ui->add(
      $ui
        ->createScript('supsystic-tables-datatables-js')
    ...
    $ui->add(
      $ui
        ->createScript('supsystic-tables-shortcode')
        ->setHookName($dynamicHookName)
    ...
    $ui->add(
      $ui
        ->createScript('supsystic-tables-notify')
        ->setHookName($dynamicHookName)
    ...
    if ($environment->isModule('tables')) {
      ...
      if ($environment->isAction('index')) {
        ...
        $ui->add($ui->createStyle('jquery-ui-theme')->setHookName($hookName)->setLocalSource('css/jquery-ui.theme.min.css')->setCachingAllowed($cachingAllowed)->setVersion($version));
      }
      ...
      if ($environment->isAction('view')) {
        ...
        $ui->add($ui->createScript('supsystic-tables-tables-view')->setHookName($hookName)->setModuleSource($this, 'js/tables.view.js')->addDependency('supsystic-tables-editor-init-js')->setCachingAllowed($cachingAllowed)->setVersion($version));
      }
    }
  }
  ...
  private function loadDataTables(SupsysticTables_Ui_Module $ui)
  {
    ...
    $ui->add(
      $ui
        ->createStyle('supsystic-tables-datatables-css')
        ->setHookName($dynamicHookName)
    ...
    $ui->add(
      $ui
        ->createScript('supsystic-tables-datatables-responsive-js')
        ->setHookName($dynamicHookName)
    ...
    $ui->add(
      $ui
        ->createScript('supsystic-tables-datatables-fixed-columns-js')
        ->setHookName($dynamicHookName)
    ...
    $ui->add(
      $ui
        ->createScript('supsystic-tables-datatables-fixed-headers-js')
        ->setHookName($dynamicHookName)
    ...
    $ui->add(
      $ui
        ->createScript('supsystic-tables-datatables-extensions-js')
        ->setHookName($dynamicHookName)
    ...
    $ui->add($ui->createScript('moment')->setHookName($dynamicHookName)->setCachingAllowed(true));

    $ui->add(
      $ui
        ->createScript('supsystic-tables-datetime-moment-js')
        ->setHookName($dynamicHookName)
    ...
    $ui->add(
      $ui
        ->createScript('supsystic-tables-datatables-natural-sort-js')
        ->setHookName($dynamicHookName)
  ...
  private function loadFeatherLight(SupsysticTables_Ui_Module $ui)
  {
    ...
    $ui->add($ui->createScript('supsystic-tables-featherlight-js')->setHookName($dynamicHookName)->setModuleSource($this, 'libraries/featherlight/featherlight.min.js')->setCachingAllowed(true)->addDependency('jquery')->setVersion('1.7.13'));

    $ui->add($ui->createStyle('supsystic-tables-tables-featherlight-css')->setHookName($dynamicHookName)->setModuleSource($this, 'libraries/featherlight/featherlight.min.css')->setCachingAllowed(true)->setVersion('1.7.13'));
  }
  ...
  private function loadRuleJS(SupsysticTables_Ui_Module $ui)
  {
    ...
    $ui->add($ui->createScript('supsystic-tables-rulejs-libs-js')->setHookName($dynamicHookName)->setModuleSource($this, 'libraries/ruleJS/ruleJS.lib.full.js'));

    $ui->add($ui->createScript('supsystic-tables-rulejs-parser-js')->setHookName($dynamicHookName)->setModuleSource($this, 'libraries/ruleJS/parser.js'));

    $ui->add($ui->createScript('supsystic-tables-rulejs-js')->setHookName($dynamicHookName)->setModuleSource($this, 'libraries/ruleJS/ruleJS.js'));
  }
  ...
  private function loadHandsontable(SupsysticTables_Ui_Module $ui)
  {
    ...
    // Styles
    $ui->add($ui->createStyle('supsystic-tables-handsontable-css')->setHookName($hookName)->setModuleSource($this, 'libraries/handsontable/handsontable.full.min.css')->setCachingAllowed(true)->setVersion('7.1.1'));

    $ui->add($ui->createStyle('supsystic-tables-rulejs-hot-css')->setHookName($hookName)->setModuleSource($this, 'libraries/ruleJS/handsontable.formula.css'));

    // Scripts
    $ui->add($ui->createScript('supsystic-tables-handsontable-js')->setHookName($hookName)->setModuleSource($this, 'libraries/handsontable/handsontable.min.js')->setCachingAllowed(true)->setVersion('7.1.1'));

    $ui->add($ui->createScript('supsystic-tables-rulejs-hot-js')->setHookName($hookName)->setModuleSource($this, 'libraries/ruleJS/handsontable.formula.js')->addDependency('supsystic-tables-handsontable-js'));
  }
  ...
  private function loadAceCssEditor(SupsysticTables_Ui_Module $ui)
  {
    ...
    $ui->add($ui->createScript('supsystic-tables-ace-editor-js')->setHookName($hookName)->setModuleSource($this, 'js/ace/ace.js')->setCachingAllowed(true)->setVersion('1.2.5'));

    $ui->add($ui->createScript('supsystic-tables-ace-editor-mode-js')->setHookName($hookName)->setModuleSource($this, 'js/ace/mode-css.js')->addDependency('supsystic-tables-ace-editor-js'));

    $ui->add($ui->createScript('supsystic-tables-ace-editor-theme-js')->setHookName($hookName)->setModuleSource($this, 'js/ace/theme-monokai.js')->addDependency('supsystic-tables-ace-editor-js'));
  }
  ...
  public function wooProAddonAds()
  {
    ...
    if (!$notice) {
      ...
      add_action('admin_notices', [$this, 'showWooAdsNotice']);
    }
  }
  ...
  public function getShortcodesList()
  {
    ...
    if ($this->getEnvironment()->isPro()) {
      ...
      $shortcodes['history_shortcode'] = [
        'name' => $config->get('shortcode_name'),
        'label' => $environment->translate('History Shortcode'),
        'attrs' => 'use_history=1',
      ];
      $shortcodes['sql_shortcode'] = [
        'name' => $config->get('shortcode_name'),
    ...
    return $dispatcher->apply('table-shortcodes-list', [$shortcodes]);
  }
