...
class SupsysticTables_Core_Module extends SupsysticTables_Core_BaseModule
{
  ...
  {
    wp_localize_script('tables-core', 'ajax_obj', [
  ...
  public function loadDataTablesNonces()
  {
    ...
    if (is_admin() && (current_user_can('administrator') || $userCanEdit)) {
      ...
      wp_register_script('dtgs_nonce', $path, [], '0.01', true);
    ...
    if (!is_admin()) {
      ...
      wp_register_script('dtgs_nonce_frontend', $path, [], '0.01', true);
  ...
  public function afterUiLoaded(SupsysticTables_Ui_Module $ui)
  {
    ...
    $ui->add($ui->createScript('jquery-ui-dialog')->setHookName($hookName));

    $ui->add($ui->createScript('jquery-contextmenu')->setHookName($dynamicHookName)->setModuleSource($this, 'js/lib/jquery.contextMenu.min.js')->setVersion('2.6.4'));

    $ui->add($ui->createStyle('jquery-contextmenu')->setHookName($dynamicHookName)->setModuleSource($this, 'css/lib/jquery.contextMenu.min.css')->setVersion('2.6.4'));

    /* Core script with common functions in supsystic.Tables namespace */
    $ui->add($ui->createScript('tables-core')->setHookName($dynamicHookName)->setModuleSource($this, 'js/core.js')->addDependency('jquery')->setCachingAllowed($cachingAllowed)->setVersion($pluginVersion));

    add_action('wp_enqueue_scripts', [$this, 'enquieAjaxUrl'], 999);
    add_action($dynamicHookName, [$this, 'addCommonPluginData'], 999);
    add_action($dynamicHookName, [$this, 'addFontsData'], 999);
    add_action($dynamicHookName, [$this, 'loadDataTablesNonces'], 1);

    /* Script for creating new table by click on main plugin tab "Add table" */
    $ui->add(
      $ui
        ->createScript('tables-create-table')
    ...
    $ui->add($ui->createScript('tables-tooltipster')->setHookName($hookName)->setModuleSource($this, 'js/jquery.tooltipster.min.js')->addDependency('jquery')->setCachingAllowed(true)->setVersion($pluginVersion));

    /* Chosen */
    $ui->add($ui->createStyle('tables-chosen')->setHookName($hookName)->setLocalSource('css/chosen.min.css')->setCachingAllowed($cachingAllowed)->setVersion($pluginVersion));
    ...
    /* iCheck */
    $ui->add($ui->createScript('tables-iCheck')->setHookName($hookName)->setLocalSource('js/plugins/icheck.min.js')->addDependency('jquery')->setCachingAllowed(true)->setVersion('1.0.2'));

    /* Supsystic UI */
    $ui->add($ui->createStyle('supTablesUI')->setHookName($hookName)->setLocalSource('css/libraries/supsystic/suptablesui.min.css')->setCachingAllowed($cachingAllowed)->setVersion($pluginVersion));

    $ui->add($ui->createStyle('tables-ui-inputs')->setHookName($hookName)->setLocalSource('css/libraries/supsystic/inputs.css')->setCachingAllowed($cachingAllowed)->setVersion($pluginVersion));

    $ui->add($ui->createStyle('tables-ui-buttons')->setHookName($hookName)->setLocalSource('css/libraries/supsystic/buttons.css')->setCachingAllowed($cachingAllowed)->setVersion($pluginVersion));

    $ui->add($ui->createStyle('tables-ui-forms')->setHookName($hookName)->setLocalSource('css/libraries/supsystic/forms.css')->setCachingAllowed($cachingAllowed)->setVersion($pluginVersion));

    $ui->add($ui->createStyle('supsystic-font-awesome')->setHookName($hookName)->setLocalSource('css/libraries/fontawesome/font-awesome.min.css')->setCachingAllowed($cachingAllowed)->setVersion('4.7.0'));
    ...
    if ($environment->isAction('index')) {
      ...
      $ui->add($ui->createStyle('jqgrid-css')->setHookName($hookName)->setLocalSource('css/libraries/jqGrid/ui.jqgrid.css')->setCachingAllowed($cachingAllowed)->setVersion('4.7.0'));
      $ui->add(
        $ui
          ->createScript('jquery-jqGrid-locale')
          ->setHookName($hookName)
      ...
      $ui->add($ui->createScript('jquery-jqGrid')->setHookName($hookName)->setLocalSource('js/libraries/jqGrid/jquery.jqGrid.min.js')->addDependency('jquery')->setCachingAllowed($cachingAllowed)->setVersion($pluginVersion));
    }
  }
  ...
  public function getModelsFactory()
  {
    if (!$this->modelsFactory) {
      $this->modelsFactory = new SupsysticTables_Core_ModelsFactory($this->getEnvironment());
    }
  ...
  private function handleMainRequest($isAjax = false)
  {
    ...
    if ($isAjax || (!$isAjax && !$isError)) {
      return call_user_func_array([$module->getController(), $actionName], [$request]);
    }
  }
  ...
  {
    $message = sprintf(
      $this->getEnvironment()->translate('Your PHP configuration has enabled "%s" directive. ' . 'This is deprecated directive and we are can not guarantee ' . 'that the plugin will work properly. To turn off this directive check the %s tutorial %s.'),
  ...
  private function makeDbUpdate($revision)
  {
    ...
    for ($i = $revision['installed']; $i <= $revision['current']; $i++) {
      ...
      try {
        ...
        wp_die(sprintf('Failed to update plugin database. Reason: %s', esc_html($e->getMessage())));
      }
    }
  ...
  private function registerTwigFunctions()
  {
    ...
    $twig->addFunction(new Twig_SupTwgDtgs_SimpleFunction('plugin_directory_url', [$this, 'getPluginDirectoryUrl']));
    $twig->addFunction(new Twig_SupTwgDtgs_SimpleFunction('build_pro_url', [$this, 'buildProUrl']));
    $twig->addFunction(new Twig_SupTwgDtgs_SimpleFunction('translate', [$this, 'translate']));
    if (function_exists('dump') && $this->getEnvironment()->isDev()) {
      $twig->addFunction(new Twig_SupTwgDtgs_SimpleFunction('dump', 'dump'));
    }
    if (function_exists('preg_replace')) {
      $twig->addFilter(new Twig_SupTwgDtgs_SimpleFilter('preg_replace', [$this, 'addPregReplaceFilter']));
    }
    $config = $this->getEnvironment()->getConfig();
    ...
    global $current_user;
    if ($current_user):
      $twig->addGlobal('DTGS_USER_NAME', $current_user->user_firstname . ' ' . $current_user->user_lastname);
