...
abstract class SupsysticTables_Core_BaseModel extends RscDtgs_Mvc_Model implements RscDtgs_Environment_AwareInterface
{
  ...
  {
    $query = $this->getQueryBuilder()->select('*')->from($this->getTable());
   ...
   * @return stdClass|null
   */
  public function get($field, $value, array $parameters = [])
  {
    $query = $this->getQueryBuilder()->select('*')->from($this->getTable())->where($field, '=', htmlspecialchars($value));
   ...
   */
  public function remove($field, $value, array $parameters = [])
  {
    $query = $this->getQueryBuilder()->deleteFrom($this->getTable())->where($field, '=', htmlspecialchars($value));
  ...
  {
    if (count($fields) === 0) {
      throw new InvalidArgumentException('The values for the insertion were not specified.');
    }

    $values = $this->beforeValuesSet($fields);

    $query = $this->getQueryBuilder()->insertInto($this->getTable())->fields(array_keys($fields))->values($values);
  ...
  {
    if (count($fields) === 0) {
      throw new InvalidArgumentException('The values for the updates were not specified.');
    }

    $values = $this->beforeValuesSet($fields);

    $query = $this->getQueryBuilder()->update($this->getTable())->fields(array_keys($fields))->values($values)->where('id', '=', (int) $id);
   ...
   * @throws RuntimeException if query failed.
   */
  public function removeById($id, array $parameters = [])
  {
    try {
      $id = (int) $id;
  ...
  protected function dispatchFilter($name, $result)
  {
    ...
    return $this->environment->getDispatcher()->apply($this->getRawTableName() . '_' . strtolower($name), [$result]);
  }
   ...
   */
  protected function populateQuery(BarsMaster_ChainQueryBuilder $queryBuilder, array $parameters)
  {
