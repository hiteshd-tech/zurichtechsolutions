...
abstract class SupsysticTables_Ui_Asset implements SupsysticTables_Ui_AssetInterface
{
  ...
  {
    if (!$this->hookName) {
      throw new UnexpectedValueException('Mandatory property "hookName" is empty.');
    }

    if (!$this->handle) {
      throw new UnexpectedValueException('Mandatory property "handle" is empty.');
    }

    add_action($this->hookName, [$this, 'enqueue']);
  }
  ...
  public function setModuleSource($module, $moduleSource)
  {
    ...
    if (is_string($module)) {
      ...
      throw new InvalidArgumentException('First parameter must be module name or the instance of the module.');
    }
  ...
  {
    if (false === ($key = array_search(strtolower($dependency), $this->dependencies, false))) {
      return false;
    }
