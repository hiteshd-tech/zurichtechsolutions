...
class SupsysticTables_ContactForm_Module extends SupsysticTables_Core_BaseModule
{
  ...
  public function call($method, $arguments = null)
  {
    if ($this->isInstalled()) {
      if (!method_exists($this->getInstance(), $method)) {
        throw new BadMethodCallException(sprintf($this->getEnvironment()->translate('Call to undefined method %1$s for %2$s.'), $method, $this->frameClass));
      }
    } else {
      return false;
    }
  ...
  {
    return $this->call('getModule', [$name]);
  }
  ...
  protected function getInstance()
  {
    ...
    return call_user_func_array([$this->frameClass, '_'], []);
  }
}
