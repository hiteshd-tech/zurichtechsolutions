...
class SupsysticTables_Migrationfree_Model_Exporter extends SupsysticTables_Core_BaseModel
{
  ...
  {
    $tables = ['columns', 'conditions', 'diagrams', 'rows', 'rows_history'];
    ...
    foreach ($ids as $i => $id) {
      ...
      print 'INSERT INTO `%prefix%tables` (' . $fields . ')' . PHP_EOL . 'VALUES' . $this->selectValues($tableName, $fields, 'id', $id) . ';' . $delimEOL . 'SET @table_id = (SELECT last_insert_id());' . $delimEOL;
  ...
  {
    $query = $this->getQueryBuilder()->select($fields)->from($table)->where($where, '=', $id);
    if ($limit > 0) {
  ...
  {
    $query = $this->getQueryBuilder()->select('count(*)')->from($table)->where($where, '=', $id);
