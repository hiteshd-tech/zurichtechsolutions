...
class Twig_SupTwgDtgs_NodeVisitor_Optimizer extends Twig_SupTwgDtgs_BaseNodeVisitor
{
  ...
  protected function optimizePrintNode(Twig_SupTwgDtgs_NodeInterface $node, Twig_SupTwgDtgs_Environment $env)
  {
    ...
    if ($exprNode instanceof Twig_SupTwgDtgs_Node_Expression_BlockReference || $exprNode instanceof Twig_SupTwgDtgs_Node_Expression_Parent) {
  ...
  protected function enterOptimizeFor(Twig_SupTwgDtgs_NodeInterface $node, Twig_SupTwgDtgs_Environment $env)
  {
    ...
    elseif ($node instanceof Twig_SupTwgDtgs_Node_Expression_Function && 'include' === $node->getAttribute('name') && (!$node->getNode('arguments')->hasNode('with_context') || false !== $node->getNode('arguments')->getNode('with_context')->getAttribute('value'))) {
      $this->addLoopToAll();
    }

    // the loop variable is referenced via an attribute
    elseif (
      $node instanceof Twig_SupTwgDtgs_Node_Expression_GetAttr &&
      (!$node->getNode('attribute') instanceof Twig_SupTwgDtgs_Node_Expression_Constant || 'parent' === $node->getNode('attribute')->getAttribute('value')) &&
      (true === $this->loops[0]->getAttribute('with_loop') || ($node->getNode('node') instanceof Twig_SupTwgDtgs_Node_Expression_Name && 'loop' === $node->getNode('node')->getAttribute('name')))
    ) {
      $this->addLoopToAll();
    }
  }
